...
s32b _finalize_p(s32b p, u32b flgs[OF_ARRAY_SIZE], object_type *o_ptr, int options)
{
    ...
    {
        p = 0;
        if (o_ptr->name1 || o_ptr->name2 || o_ptr->art_name) p = 1;
    }
...
s32b bag_cost(object_type *o_ptr, int options)
{
  ...
  if (options & COST_REAL) obj_flags(o_ptr, flgs);
  else obj_flags_known(o_ptr, flgs);
...
s32b new_object_cost(object_type *o_ptr, int options)
{
    ...
    else if (o_ptr->tval == TV_BAG)    return     bag_cost(o_ptr, options);
