...
static void _add_aux(inv_ptr inv, obj_ptr obj, slot_t slot)
{
    ...
    if(!silent_carry_hack) copy->marked |= OM_DELAYED_MSG;
...
slot_t inv_combine(inv_ptr inv, obj_ptr obj)
{
    ...
    for (slot_t slot = 1; slot < vec_length(inv->objects); slot++)
    {
          ...
          && dest->number + obj->number <= OBJ_STACK_MAX )
        {
            ...
            if(!silent_carry_hack) dest->marked |= OM_DELAYED_MSG;
            return slot;
        }
    }
    assert(obj->number);
...
int inv_combine_ex(inv_ptr inv, obj_ptr obj)
{
    ...
    for (slot_t slot = 1; slot < vec_length(inv->objects); slot++)
    {
        ...
        if (obj_can_combine(dest, obj, inv->type))
        {
            ...
            ct += n;
            if (n && !silent_carry_hack) dest->marked |= OM_DELAYED_MSG;
            if (!obj->number) break;
        }
    }
    return ct;
...
void inv_display(inv_ptr inv, slot_t start, slot_t stop, obj_p p, doc_ptr doc, int flags)
{
    ...
    for (slot_t slot = start; slot <= stop; slot++)
    {
        ...
        else
        {
            ...
            bool charging = (flags & INV_SHOW_FAIL_RATES) && !obj_is_device(obj) && obj->timeout > 0;
            object_desc(name, obj, charging ? 0 : OD_COLOR_CODED);
